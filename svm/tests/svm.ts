  ...
  describe("messages", () => {
    ...
    const createAndPostVaa = async (
      ...
      const w = new Wormhole("Mainnet", [SolanaPlatform], {
        chains: { Solana: { contracts: { coreBridge: WORMHOLE_PROGRAM_ID } } },
      });
      ...
      const vaaKey = utils.derivePostedVaaKey(
        WORMHOLE_PROGRAM_ID,
        Buffer.from(rawVaa.hash),
      );

      try {
        const postedVaa = core.postVaa(sender, rawVaa);
        await signSendWait(ctx, postedVaa, signer);
        return { vaaKey, vaaHash: Array.from(rawVaa.hash) };
      } catch (error) {
    ...
    describe("recv_message", () => {
      ...
      it("successfully attests valid VAA from registered peer", async () => {
        ...
        const { vaaKey, vaaHash } = await createAndPostVaa(
          expectedPayload,
          ethEmitter,
        );
